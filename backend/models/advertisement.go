...
// Model bisnis: sistem iklan marketplace dengan approval workflow Admin.
// Advertiser bisa UMKM (iklan produk sendiri) atau External (pihak ketiga).
// Flow: PENDING → APPROVED (Admin) → ACTIVE (tayang) → EXPIRED (selesai durasi).
// Service fee 5% ditambahkan ke price. Metrics tracking (impressions, clicks, CTR).
